...
class student{
    ...
    // constructor() 생성자 함수를 작성안하면, constructor() 빈 생성자 함수가 자동으로 생긴다
    ...
    getInfo(){
        return "나이는 " + this.age + "살임. 핸드폰 번호는 " + this.phone + "이고, 사는 곳은 " + this.city + "임"
    }
}
...
class Character{
    ...
    // 클래스의 인스턴스에서 호출 x
    // static 메소드는 클래스를 동적할당 할때마다 생성되지 않는다. 한개만 있다.
...
console.log(ca);
// console.log(ca.getAtk(1)); // 안됨. 인스턴스에서 호출 불가
console.log(Character.getAtk(ca)); // static 호출하는 방법


class Product{
    constructor(name, price){ // 생성자 함수부터 만들고 시작하자.
        this.name = name;
        this. price = price;
    }
    // getter, setter
    ...
    // 클래스의 값에 접근할때 직접 변수에 접근하는 형태가 아닌
    // get과 set을 통한 겁근방법
    // 내부구조를 캡슐화하는데 좋다.
