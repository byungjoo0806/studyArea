...
// 헤더의 내용은 전달하는 메시지(데이터)의 정보를 전달하고,
// 바디에는 전달하는 데이터의 내용이 들어있다.
...
// 요청 메소드
// GET : 데이터의 요청을 의미 (필요한 데이터를 응답 받는것); 데이터를 가져오는것
...
// 우리가 배울건 1.1 버전
// 1.1 : WWW 에서 사용되는 기본 프로토콜; HTTP 버전
// 1997년도에 도입이 되었고, 지금까지도 많이 사용중

// body의 내용
// Buffer.from 메소드를 사용해서 문자열을 바이트 데이터로 변환 해준다.
// 변환하는 이유는 HTTP 응답은 바이트 데이터로 전송되기 때문
// body 문자열을 그대로 작성해버리면 인코딩에 문제가 생길수가 있기 때문
...
// 1xx : 거의 없어요..
// 2xx : 응답 성공, 요청 성공에 대한 내용
// 3xx : 리다이렉트
// 4xx : 페이지가 없어 (404 : 요청한 페이지가 없어)
// 5xx : 서버 터진거

// 가장 많이 사용하는 상태코드는 200(성공)과 404(실패)

// Connection : 클라이언트와 서버의 연결 상태
// keep-alive 속성 : 클라이언트가 다음 요청을 보낼때까지 연결 유지

// Keep-Alive : 연결을 유지하는 시가능ㄹ 지정
// timeout=4 : 연결을 4초동안 유지하고, 4초 후에 연결을 종료

// Content-type : 전송되는 데이터의 타입
// text/html : HTML 타입의 데이터 전송
