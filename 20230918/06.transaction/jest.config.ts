...
const config : Config.InitialOptions = {
    ...
    // 루트 경로에서 모든 폴더에 모든 파일 이름의 패턴이 test.js ghrdms test.ts
    testMatch : ['<rootDir>/**/*.test.(js|ts)'],

    // 3. 모듈의 이름에 대한 별칭 설정 : @core
    // 별칭으로 지정된 @core 를 어떻게 경로를 바꿔줄거냐
    // ^@core == @core/**/* 로 시작하는 별칭은 루트경로에 src/core 의 경로까지
    moduleNameMapper : {
        "^@core/(.*)$" : "<rootDir>/src/core/$1"
    },

    // 4. 테스트 환경 설정 : node 환경에서 실행 시키게끔
