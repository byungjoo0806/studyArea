...
enum MessageType {
    ...
    // 마지막 블록을 요청할때
    lastBlock = 0, // string 문자로 해도 되는데, 오타가 발생할 수도 있어서 number로 오류가 최대한 없게
...
class P2P extends Chain {
    // Chain 클래스를 상속 받아서 Chain 에 있는 메소드를 사용할 수 있다.
    ...
    connectSocket(socket : any, type? : MessageType) : void {
        // 소켓을 연결하면
        // 하나의 포트가 동적으로 생성이 되고, 그 포트에 소켓이 있는데
        // socket에는 고유의 포트가 들어있는 상태
        // 충돌방지를 위해 어플리케이션 or 서비스 연결을 하면 동적으로 포트를 지정해준다. (고유 포트)
        this.sockets.push(
            `${socket._socket.remoteAddress} : ${socket._socket.remotePort}`
        );
        // socket.send() 메소드를 호출하면 이벤트가 실행된다.
        // message 이벤트 실행
    ...
    listen(port : number) : void {
        ...
        server.on("connection", (socket : WebSocket) => {
            ...
            // 연결한 소켓을 배열에 추가해주고 message 이벤트도 등록
