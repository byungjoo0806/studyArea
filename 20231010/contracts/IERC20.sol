...
interface IERC20 {
    ...
    // {0x11111111111111111111111111 : 100, 0x111111111111111111111112 : 200}
    // {0x11111111111111111111111111 : {0x111111111111111111111112 : 50}}

    // 소유권을 제 3자에게 위임하는 함수
    function approve(address spender, uint amount) external returns (bool);

    // 소유권을 관리하는 토큰을 보내는 함수; 위임받은 소유권이 있는지 확인을하고 보내는 함수
    ...
    // Transfer 함수를 실행했을때 이벤트 함수를 실행해서 발생하는 이벤트를 트랜잭션 로그에 확인해볼 수 있다.
    event Transfer(address from, address to, uint value);

    // approve 함수가 실행됬을때 이벤트 함수도 실행을 해서 로그를 확인해볼 수 있다.
